...
public class FindKthNum {
    public static void main(String[] args) {
        ...
        int result = findNum(nums, 0, nums.length - 1, 0);

        System.out.println(result);
    }

    private static int findNum(int[] nums, int start, int end, int k) {
        if (start >= end || k > nums.length) throw new IllegalArgumentException();
        int p = partition(nums, start, end);
        if (p + 1 == k) {
            return nums[p];
        } else if (k < p + 1) {
            return findNum(nums, start, p - 1, k);
        } else if (k > p + 1) {
            return findNum(nums, p + 1, end, k);
        }
        return 0;
    }
